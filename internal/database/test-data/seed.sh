...
sqlite3 "$DB_PATH" "DELETE FROM plant_species_images WHERE plant_species_id IN (SELECT id FROM plant_species WHERE user_id IS NULL AND deleted_at IS NULL);"
...
sqlite3 "$DB_PATH" "DELETE FROM plants WHERE user_id IS NULL;"
sqlite3 "$DB_PATH" "DELETE FROM plant_species WHERE user_id IS NULL AND deleted_at IS NULL;"
